...
for i in range(len(vpc_list)-1):
    vpc1 = vpc_list[i]
    for j in range(len(vpc_list)-1-i):
        vpc2 = vpc_list[i+j+1]
        #command = f"echo './{exec_file} " + " ".join(f"{value}" for value in vpc1.__dict__.values()) + " " + " ".join(f"{value}" for value in vpc2.__dict__.values()) + "'"
        command = f"./{exec_file} " + " ".join(f"{value}" for value in vpc1.__dict__.values()) + " " + " ".join(f"{value}" for value in vpc2.__dict__.values())
        os.system(command)